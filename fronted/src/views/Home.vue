...
onMounted(() => {
  newsStore.getCategories().then(() => {
    const initialCategory = displayCategories.value[activeTab.value]
    if (!initialCategory) {
      newsStore.getNewsList()
      return
    }

    if (newsStore.currentCategory !== initialCategory.id) {
      newsStore.changeCategory(initialCategory.id)
      return
    }
